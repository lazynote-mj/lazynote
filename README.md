...
어디서부터 시작할지 모르겠으면 `/lazynote:ask-lazynote`를 먼저 불러본다 — 준비 상태에 따라 어느 스킬로 갈지 안내한다.
...
| 한국어 맞춤법·띄어쓰기·호응 교정과 자연스러운 윤문 — 아티클·에세이뿐 아니라 보고서·이메일·안내문 같은 업무 문서도 포함 | `lazynote-korean-editing` | "맞춤법만 고쳐줘", "이 보고서 문장 자연스럽게 다듬어줘" |
| 아티클의 논지를 설명하는 도표 또는 표지·썸네일 이미지 | `lazynote-article-images` | "이 글에 맞는 이미지 만들어줘", "이 도표를 최신 원고에 맞춰줘" |
| 시리즈·책을 처음 시작해서 방향이 아직 없다 | (위 네 개 집필 스킬에서 새 기획 시) `shared/intake.md` 기획 인터뷰 | "시리즈 어떻게 시작해야 할지 모르겠다", "책 기획 좀 도와줘" |

새 시리즈·책을 기획하며 목적·독자·용어·전체 목차가 저장소에 없으면, 위 표의 앞 네 개 집필 스킬(`lazynote-case-writing`·`lazynote-title-outline`·`lazynote-author-writing`·`lazynote-persona-writing`)에서 기획 인터뷰부터 진행한다. `lazynote-korean-editing`·`lazynote-article-images`처럼 이미 있는 원고나 이미지를 다루는 스킬에는 이 인터뷰가 필요하지 않다.
